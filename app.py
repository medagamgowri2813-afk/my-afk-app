...
def extract_text_from_file(uploaded_file):
    """Extracts text from TXT or PDF file."""
...
if page == "🏠 Home":
    ...
    st.header("Chat with Smart City Assistant")
    ...
    if st.button("What are the key components of a smart city?"):
        q = "What are the key components of a smart city?"
        a = "Smart cities include IoT, smart transport, energy systems, governance, and sustainability."

        st.session_state.messages.append({"role": "user", "content": q})
        st.session_state.messages.append({"role": "assistant", "content": a})
        st.rerun()

    if st.button("How can IoT improve waste management?"):
        q = "How can IoT improve waste management?"
        a = "IoT helps using smart bins, fill-level sensors, and optimized waste collection routes."

        st.session_state.messages.append({"role": "user", "content": q})
        st.session_state.messages.append({"role": "assistant", "content": a})
        st.rerun()

    if st.button("Suggest green infrastructure policies for urban areas."):
        q = "Suggest green infrastructure policies for urban areas."
        a = "Green roofs, rainwater harvesting, urban forests, and electric public transport are key policies."
    ...
    if user_input := st.chat_input("Type your question here..."):
        ...
        # demo response (replace with AI model later if needed)
        response = f"🤖 Smart City Assistant: {user_input}"
    ...
    with st.form("citizen_feedback_form", clear_on_submit=True):
        ...
        category = st.selectbox(
            "Category",
            ["General Feedback", "Water & Sanitation", "Roads & Infrastructure",
             "Environment & Green Spaces", "Public Safety", "Waste Management",
             "Energy", "Other"]
        )
    ...
    # -------- HANDLE SUBMIT (IMPORTANT OUTSIDE FORM) --------
    if submitted:
        if not description or len(description) < 10:
            st.error("Please provide at least 10 characters in description.")
        else:
            text = description.lower()

            # Department detection
            if "street light" in text or "electricity" in text:
                department, priority = "Electricity Department", "High"

            elif "road" in text or "pothole" in text:
                department, priority = "Roads Department", "High"

            elif "garbage" in text or "waste" in text:
                department, priority = "Waste Management", "Medium"

            elif "water" in text:
                department, priority = "Water Department", "High"

            else:
                department, priority = "General Department", "Low"

            # Store in session (fix repeated submit issues)
    ...
    # -------- OPTIONAL: SHOW LAST SUBMISSION --------
    if "last_feedback" in st.session_state:
        with st.expander("📌 Last Submitted Complaint"):
            data = st.session_state["last_feedback"]
            st.write(data)